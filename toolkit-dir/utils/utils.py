...
import numpy as np # 나
# 나    
def posPrediction(prevFrames: list):
    prev = prevFrames.copy()
    prev.sort()
    
    totalPosDiff = np.array([0.0, 0.0])
    totalSizeDiff = np.array([0.0, 0.0])
    for i in range(len(prev)-1):
        frameDiff = prev[i+1][0] - prev[i][0]
        pos1, size1 = [prev[i][1], prev[i][2]], [prev[i+1][3], prev[i+1][4]]
        pos2, size2 = [prev[i+1][1], prev[i+1][2]], [prev[i+1][3], prev[i+1][4]]
        cpos1 = [(pos1[0]+size1[0]/2)/(frameDiff+(len(prev)-1)), (pos1[1]+size1[1]/2)/(frameDiff+(len(prev)-1))]
        cpos2 = [(pos2[0]+size2[0]/2)/(frameDiff+(len(prev)-1)), (pos2[1]+size2[1]/2)/(frameDiff+(len(prev)-1))]
        
        cpos1, size1, cpos2, size2 = np.array(cpos1), np.array(size1), np.array(cpos2), np.array(size2)

        cposDiff = (cpos2-cpos1) #/frameDiff
        sizeDiff = (size2-size1) #/frameDiff
        totalPosDiff += cposDiff
        totalSizeDiff += sizeDiff
        
    #posPred = totalPosDiff/(len(prev)-1)
    #szPred = totalSizeDiff/(len(prev)-1)
    return totalPosDiff, totalSizeDiff
...
def trajectory_overlaps(t1: list, t2: list):
    ...
    for i, (reg1, reg2) in enumerate(zip(t1, t2)):
        if len(reg1) > 1 and len(reg2) > 1: # failure가 아닌 것만으로 averge overlap 계산
...
def count_failures(overlaps, overlap_valid):
    failures = 0
