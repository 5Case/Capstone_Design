...
from .rpnUtil import get_subwindow_tracking

import matplotlib.pyplot as plt
from .rpnUtil import torch_to_img

def generate_anchor(total_stride, scales, ratios, score_size):
    anchor_num = len(ratios) * len(scales) # 5*1 = 5
    anchor = np.zeros((anchor_num, 4),  dtype=np.float32) # 5x4행렬
    size = total_stride * total_stride # size = 64
    count = 0
    for ratio in ratios: # 5번 loop
    ...
    return anchor # xx, yy는 anchor stride별 x, y 좌표
...
def tracker_eval(net, x_crop, target_pos, target_sz, window, scale_z, p):
    ...
    #내가 추가
    #print(f' score[best]: {score[best_pscore_id]}, pscore[best]: {pscore[best_pscore_id]}\n')
...
def SiamRPN_init(im, target_pos, target_sz, net, net_name):
    ...
    net.temple(z.cuda()) # template frame
