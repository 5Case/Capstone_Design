...
class SiamRPN(nn.Module):
    ...
    def xcorr(self, z, x, channels):
        ...
        for i in range(x.size(0)): # 나, x.size(0) = 1 / 0차원 size 확인
        ...
        # unsqueeze(0) -> 첫번째 차원에 1인 차원 추가 즉, [...] -> [[...]]    
            #print(x[i, :, :, :].unsqueeze(0), x[i, :, :, :].unsqueeze(0).shape) # 1, 256, 20, 20 / 1, 256, 20, 20
            #print(z[i, :, :, :].unsqueeze(0).view(channels, self.model.features.feature_channel, kernel_size, kernel_size).shape) # torch.Size([10, 256, 4, 4]), torch.Size([20, 256, 4, 4])
        #print(channels, self.model.features.feature_channel, kernel_size, kernel_size) #-> 10 256 4 4 / 20 256 4 4
            #print(out, out[0].size(), '\n') # 1, 10, 17, 17 / 1, 20, 17, 17
    ...
    def forward(self, detection):
        ...
        #print(pred_score.shape, pred_regression.shape) # torch.Size([1, 10, 17, 17]) torch.Size([1, 20, 17, 17])
        #print(type(pred_regression), type(pred_score)) # torch.Tensor
...
# 내가 추가
class SiamRPN_MOT16(SiamRPN):
    def __init__(self, tracker_name='SiamRPN_MOT16'):
        super().__init__(tracker_name)
        self.cfg = {'lr': 1e-3, 'window_influence': 0.01, 'penalty_k': 0.22, 'instance_size': 255, 'adaptive': False}   
...
class SiamRPN_MOT16(SiamRPN):
    def __init__(self):
        super().__init__(size=2, feature_out=512)
        self.cfg = {'lr': 0.295, 'window_influence': 0.42, 'penalty_k': 0.055, 'instance_size': 255, 'adaptive': True}
'''